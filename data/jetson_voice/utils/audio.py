...
def AudioInput(wav=None, mic=None, sample_rate=16000, chunk_size=16000):
    ...
    """
    # let's look at AudioWavStream first
    print("[jetson_voice/utils/audio.py] Loading an instance of AudioInput. Returns AudioWavStream or AudioMicStream")
...
class AudioWavStream:
    ...
    def __init__(self, filename, sample_rate, chunk_size):
        ...
        print("[jetson_voice/utils/audio.py] Finished initialising audio playback stream from .wav file:")
        print(f"[jetson_voice/utils/audio.py] self.filename={self.filename}, self.chunk_size={self.chunk_size}, self.sample_rate={self.sample_rate}, self.samples={self.samples}, len(self.samples)={len(self.samples)}, self.position={self.position}")
    ...
    def next(self):
        print("[jetson_voice/utils/audio.py] Iterating audio playback stream")
        if self.position >= len(self.samples):
            print("[jetson_voice/utils/audio.py] self.position >= len(self.samples). Stopping stream")
            return None
        
        print("[jetson_voice/utils/audio.py] Fetching next chunk...")
        chunk = self.samples[self.position : min(self.position + self.chunk_size, len(self.samples))]
        
        if len(chunk) < self.chunk_size:
            print("[jetson_voice/utils/audio.py] len(chunk) < self.chunk_size, padding chunk with zeros")
    ...
    def __iter__(self):
        # this initialises the iterator method
        self.position = 0
        return self
