...
def load_resource(resource, factory_map, *args, **kwargs):
    ...
    """
    print("[jetson_voice/utils/resource.py] Loading an instance of a resource from a config or service name.")
    # resource a str, factory_map a dict
    if isinstance(resource, str):
        root, ext = os.path.splitext(resource)
        # ext represents extension e.g. .json
        if len(ext) > 0:
            ext = ext.lower()
            print(f"[jetson_voice/utils/resource.py] resource: {resource}, root: {root}, ext: {ext}")            
            print("[jetson_voice/utils/resource.py>jetson_voice/utils/config.py] Passing config.json filepath to create ConfigDict object")
            if ext == '.json':
            ...
            print(f"[jetson_voice/utils/resource.py<jetson_voice/utils/config.py] Loaded config: {config} of type: {type(config)}")
        else:
            # if resource is an empty string
    ...
    print(f"[jetson_voice/utils/resource.py] Setting config.backend as tensorrt")
    ...
    print("[jetson_voice/utils/resource.py] Intialising ASREngine from resource.py")
    class_name = factory_map[config.backend].rsplit(".", 1)
    class_type = getattr(importlib.import_module(class_name[0]), class_name[1])

    print(f"[jetson_voice/utils/resource.py] class_name: {class_name}, of type: {type(class_name)}")
    print(f"[jetson_voice/utils/resource.py] class_name: {class_type}, of type: {type(class_type)}; where class_type = getattr(importlib.import_module(class_name[0]), class_name[1])")
...
def load_model(config, dynamic_shapes=None):
    ...
    # here we will load either onnxruntime or tensorrt backend
    class_name = factory_map[config.backend].rsplit(".", 1)
    class_type = getattr(importlib.import_module(class_name[0]), class_name[1])
    
    logging.info(f"loading model '{config.model_path}' with {factory_map[config.backend]}")
    logging.debug(class_type)
    # here we will return the loaded model object
